...
class Write_file():
    ...
    def write2(self,data,all_vp,index_customer_in_all_vp,amount_depot,pos,index_connect):
        ...
        return file.name 
    def write_sol(self,cost,sol,path,cus_pos,map_loc):
        root = Tk()
        root.withdraw()
        file  =  filedialog.asksaveasfile(filetypes=[('text file','*.txt')],defaultextension='.txt',title='Save file as',mode='w')
        root.destroy()
        ...
        file.write(str(cost)+'\n')
        file.write(str(sol)+'\n\n')
        file.write('Result of postion from fleet management is referred to  800x800 pixel'+'\n')
        file.write(f'Fleet Result       : {path}\n')
        file.write(f'Depot,Customer Position  : {cus_pos}\n')
        file.write(f'Map Location       : {map_loc}\n')
        file.close()
